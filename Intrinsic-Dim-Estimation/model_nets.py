...
def test(R_net, D_net, testLoader_cor, device):
    """
    Calculate the the mutual information between the features and corresponding targets
    after representation learning based on the DataLoader object testLoader_cor.
    Args:
        R_net (nn.Module): The learned feature mapping, which is an nn.Module object in device.
        D_net (nn.Module): The learned mi-discriminator mapping, which is an nn.Module object in device.
        testLoader (DataLoader): An DataLoader object.
        device: Computation device: CPU or GPU
    """
    target, data, _ = next(iter(testLoader_cor))
    R_net.eval()
    D_net.eval()
    with torch.no_grad():
        ...
        permuted_y_data = target[index, :]
        MI_loss = torch.mean(torch.exp(D_net(latent, permuted_y_data))) - torch.mean(D_net(latent, target))
    return MI_loss.cpu().detach().numpy().item()
